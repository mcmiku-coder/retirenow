...
/**
 * Page A: Monte Carlo Model Overview
 */
export const generateMonteCarloOverview = (pdf, simulationResult, language, pageNum, totalPages) => {
    pdf.addPage();
    let yPos = addPageHeader(
        pdf,
        language === 'fr' ? 'Modèle Monte Carlo - Méthodologie' : 'Monte Carlo Risk Model - Methodology',
        null,
        20
    );
    yPos += 15;
    ...
    pdf.text(language === 'fr' ? 'Instruments Simulés' : 'Simulated Instruments', 20, yPos);
    yPos += 8;

    const instruments = simulationResult.investedAssetsDetails || [];
    const instrumentRows = instruments.map(inst => [
        inst.name || 'Unknown',
        formatCurrency(inst.initialValue),
        inst.performanceData ? `${inst.performanceData.length} months` : 'N/A'
    ]);
    ...
    autoTable(pdf, {
        ...
        head: [[
            ...
            language === 'fr' ? 'Valeur Initiale' : 'Initial Value',
    ...
    // A2 - Simulation Settings
    pdf.setFontSize(14);
    pdf.setFont('helvetica', 'bold');
    pdf.text(language === 'fr' ? 'Paramètres de Simulation' : 'Simulation Settings', 20, yPos);
    yPos += 8;
    ...
    const settings = [
        `Iterations: 5000+`,
        `Step: Monthly`,
        `Horizon: ${Math.round(simulationResult.horizonMonths / 12)} years`,
        `Return Model: Geometric Brownian Motion`,
        `Correlation: Historical Covariance Matrix`,
        `Reported Percentiles: P5 & P10 ONLY (Conservative)`,
        `Logic: B(t) = D(t) - P(t), T(t) = B(t) + MC(t)`
    ];
    ...
    // A3 - Covariance Matrix
    if (simulationResult.stats && simulationResult.stats.covMatrix) {
        pdf.setFontSize(14);
        pdf.setFont('helvetica', 'bold');
        pdf.text(language === 'fr' ? 'Matrice de Covariance' : 'Covariance Matrix', 20, yPos);
        yPos += 8;

        const covMatrix = simulationResult.stats.covMatrix;
        const names = instruments.map(i => i.name.substring(0, 15));

        const covBody = covMatrix.map((row, i) => {
            const rowData = [names[i]];
            row.forEach(val => rowData.push(val.toFixed(6)));
            return rowData;
        });
